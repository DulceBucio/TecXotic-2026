...
class WriteLockedFile(object):
    '''Class for wrapping an existing file with the mavlink message'''
...
class mavactive(object):
    '''Class for managing the mavlink connection'''
    def __init__(self, connection, type_=mavlink.MAV_TYPE_GENERIC,
        ...
        '''Initialises the heartbeat thread'''
        ...
        # Replace default file with the thread locking one
        self.connection.mav.file = WriteLockedFile(self.connection.mav.file)
        # set up the kill event to initialise the heartbeat thread cleanly
        self._kill = Event()
